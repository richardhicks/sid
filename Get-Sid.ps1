...
.COPYRIGHT Copyright (C) 2024-2025 Richard M. Hicks Consulting, Inc. All Rights Reserved.
...
.SYNOPSIS
    Gets the Security Identifier (SID) of the current user or the local computer's domain account.

.PARAMETER SidType
    Specifies the type of SID to return - User or Machine. The default is User.
...
.EXAMPLE
    ...
    Returns the SID for the current computer's domain account.

.DESCRIPTION
    Use this function to retrieve the SID of the current user or the local computer. The user SID is read directly from the current process token. The machine SID is the SID of the computer's domain account (not the local machine SID) and requires the computer to be domain-joined.

.INPUTS
    None. This function does not accept pipeline input.

.OUTPUTS
    PSCustomObject with Principal and SID properties.
...
# SID translation is only supported on Windows
If ($PSVersionTable.PSEdition -eq 'Core' -and -not $IsWindows) {

    Throw 'Get-Sid is only supported on Windows.'
...
Write-Verbose "Identifying SID for $($SidType.ToLowerInvariant()) $Principal..."

Try {

    If ($SidType -eq 'User') {

        # Read the SID directly from the current process token - authoritative and requires no name resolution
    ...
    Else {
        ...
        # Resolves the computer's domain account - requires domain membership
        $Id = [System.Security.Principal.NTAccount]::new($Principal)
...
Catch {
    ...
    If ($SidType -eq 'Machine') {

        $Message = "Unable to translate machine account $Principal to a SID. Verify this computer is domain-joined and a domain controller is reachable."

    }

    Else {

        $Message = "Unable to translate user $Principal to a SID."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCJSvU1ehADBDiM
# Z2ZBEuwYzrVKRw98/JyKgDW6oazPNqCCH6YwggWNMIIEdaADAgECAhAOmxiO+dAt
...
# DjAMBgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBCAbrp9GJJSdPe9MlsK/Z7nA
# 3/HTjlT2g3Agbv/O/Va1azALBgcqhkjOPQIBBQAERzBFAiEA2GyE9462gCdTkpxg
# xes5bqbqgzGcr1KCT6HD26M9RIcCIGO8ygd/H9A+aTwP6ZKQx2KG7PVGew8l/bC9
# zfvolQWZoYIDJjCCAyIGCSqGSIb3DQEJBjGCAxMwggMPAgEBMH0waTELMAkGA1UE
# BhMCVVMxFzAVBgNVBAoTDkRpZ2lDZXJ0LCBJbmMuMUEwPwYDVQQDEzhEaWdpQ2Vy
# dCBUcnVzdGVkIEc0IFRpbWVTdGFtcGluZyBSU0E0MDk2IFNIQTI1NiAyMDI1IENB
# MQIQCoDvGEuN8QWC0cR2p5V0aDANBglghkgBZQMEAgEFAKBpMBgGCSqGSIb3DQEJ
# AzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTI2MDczMTIyMDk1NVowLwYJ
# KoZIhvcNAQkEMSIEIIZij8F85JCtHGn2aCUJjRzcBF1EfCHFnflZZGHdbqdeMA0G
# CSqGSIb3DQEBAQUABIICAArwMv0Y5ovt44ZVR3zYrId40tVQpxstJWerN9W1tuAS
# isrWiGq+EbcRCcHpOF5EAsLxwxf52G3Mfyov2aqCojHp10HjGb0wnHZplyNm8mtR
# 9loQwY+M0EpeipxqWcI1FDRLxVzdpQkl5LjwL8DUj9Q9RcFfkk0uctZrHh6V28T4
# BlF1MxDwTWFQjc3SeJy4jGn0qIcLngnVOVkDxgAKeOm4gn+hfPBsHIX95wDnYCQS
# uo4/z78AxmyJcjO/GHteBq9/U9i7sJRr/oVCQxRUPhhA6KXTysSHNvKShPxXsDmD
# JaEqBAPUT5xnzfCheOUy0vQlQQePAqJgO1ehEcdxeomBCsUhKRha1+HiWKsGR0eK
# 9jqVrLseHx/WDzEXen2GMyVf7FSo3JWWvKz85H3/g2ZCXBtpHqnMQiLmNwPvOJ7O
# 7xmvY8MExv8XNesh2AgcAPZ4X3cVOvdp3WVziwPZEAh6wtX/LZh+UjCyOUHSnRgD
# qbTtj8Noen39safe74BbWCjm67RIhZKieWaPmKZFT3Bd7eSkya5mQLMytRaxmMau
# yZdrc0HeOubbyPmpUp5k+D/3JdcaS2i11i/4EjwbrZlUsz6TkCrX59qBy48efr2A
# MjbCsW9Z4ABCsMmTm8JyPwaYZLkuFx0aeqULZ5ZzjjcnBEm9GsnRBlqB8djy1U5h
